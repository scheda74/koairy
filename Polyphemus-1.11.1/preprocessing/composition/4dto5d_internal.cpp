...
int main(int argc, char** argv)
{
  ...
  Data<real,3> composition(Ncomposition,Ngroup,2);
  ExtStream composition_data(composition_data_file);
  string fraction_sets;
  int i=0;
  int j=0;
  int k=0;
  while(composition_data.GetElement(fraction_sets))
  {
    //cout<<fraction_sets<<endl;
    composition(i,j,k)=convert<real>(fraction_sets);
    //cout<<i<<j<<k<<composition(i,j,k)<<endl;
    k++;
    if(k==2)
    {
      k=0;
...
// 	         <<" ("<<z<<","<<y<<","<<x<<")"<<endl;	 
    ...
    //compute fraction of each group and locate composition of each grid
   real frac_group;
   Data<real, 1> fraction(Ngroup);
    Data<real, 3> Composition_grid(GridZ_out, GridY_out,GridX_out);
     Composition_grid.SetZero();
      for(int z = 0; z < Nz_out; z++)
	 for(int y = 0; y < Ny_out; y++)
	    for(int x = 0; x < Nx_out; x++)
	    {
		...
		for(int g = 0; g < Ngroup; g++)
		{
		  if(Conc_total(z,y,x)>0.0)
		  {
		    ...
		    fraction(g)=frac_group;
		    //cout<<frac_group<<"="<<Conc_group(g,z, y,x)<<"/"<<Conc_total(z,y,x)<<" ("<<z<<","<<y<<","<<x<<")"<<endl;
		  }
		  else
		    frac_group=0.0;
		  if(composition(i,g,0)>0)
		  {
		    if(frac_group>composition(i,g,0)&&frac_group<=composition(i,g,1))
		      counter++;
		  }
		  else
		  {
		...
		if(counter==Ngroup)
		{
		  Composition_grid(z,y,x)=i;
		  succf=1;
		}
	      }
	      if(!succf)
	      {
		cout<<"Error missing composition!"<<endl;
		cout<<Composition_grid(z,y,x)<<" "<<z<<" "<<y<<" "<<x<<endl;
		cout<<fraction(0)<<fraction(1)<<fraction(2)<<fraction(3)<<fraction(4)<<endl;
		return 2;
	      }
	    }
    ...
    while (aerosol_species2.GetElement(aerosol_names))
    {
	 ...
	 for(int y = 0; y < Ny_out; y++)
	    for(int x = 0; x < Nx_out; x++)    
	    {
	      ...
	      //cout<<g<<" "<<z<<" "<<y<<" "<<x<<endl;
	      Conc_ic_out_compos(g,z,y,x)=Conc_ic_out(z,y,x);
	    }
//       if(aerosol_names=="Number")
//       cout<<string(tostring)<<" "<<z<<" - "<<g<<endl;
      }
	  // Writes output files.
