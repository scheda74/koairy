...
int main(int argc, char** argv)
{
  ...
  config.PeekValue("Species_aero", species_file);
  config.PeekValue("Molecular_weights_aero", molecular_weights);
  ...
  config.FindFromBeginning("[aerosol_size_distribution]");
  int Ns_aero_in ; // Number of aerosols species in Mozart
  vector<vector<string> > bulk_aerosol_parameter;

  while (!config.IsEmpty() && config.PeekElement()[0] != '[')
    {
      bulk_aerosol_parameter.push_back(split(config.GetLine(),
  ...
  for (i = 0; i < Ns_aero_in; i++)
    {
      Dmean_aerosol(i) = to_num<real>(bulk_aerosol_parameter[i][1]); // attention unites
      Sigma_aerosol(i) = to_num<real>(bulk_aerosol_parameter[i][2]); // attention unites
    }
  ...
  config.FindFromBeginning("[speciation_aero]");
  ...
  for (i = 0; i < Ns_aero_in; i++)
    {
      ...
      aerosol_mozart_name.push_back(aerosol_spec[i + 2][0]);
    }
  ...
  /*Displays the aerosol speciation
    for (i = 0; i < Ns_aero_in; i++){
    for (j = 0; j < Ns_polair; j++){
  ...
  /*DELETING MOZART MAP OF SPECIES SECTION ending at species.close()*/
  ...
  for (file_index_tmp = 0;
       file_index_tmp < NumberOfDays; file_index_tmp++)
    {
      ...
      /*Output species weights for aerosol species*/
      ...
      /* BOUNDARY CONCENTRATIONS FOR AEROSOLS SPECIES*/

      cout << "Boundary concentrations for aerosol species: " << endl;
      ...
      for (ipol = 0; ipol < Ns_polair; ipol++)
        {
          ...
          cout << "weights_map.find(" << aerosol_polair_name[ipol] << ")=" << to_str(w_135) << endl;
          ...
          for (s = 0; s < Nbin; s++)
            {
              ...
              for (isp = 0; isp < Ns_aero_in; isp++)
                {
                  cout <<  "aerosol_speciation(" << aerosol_mozart_name[isp] << "," << aerosol_polair_name[ipol] << ") =" << aerosol_speciation(isp, ipol) << endl;
                }
              ...
              /*RegularGrid<real> GridT_out_2(4);

                Data<real, 4> Conc_out_x_extract(GridT_out_2, GridZ_out,
                GridY_out, GridX_interf_out);
                Data<real, 4> Conc_out_y_extract(GridT_out_2, GridZ_out,
                GridY_interf_out, GridX_out);
                Data<real, 4> Conc_out_z_extract(GridT_out_2, GridZ_interf_out,
                GridY_out, GridX_out);

                Conc_out_x_extract.SubData(Conc_out_x,
                Range(0,3),
                Range::all(),
                Range::all(),
                Range::all());

                Conc_out_y_extract.SubData(Conc_out_y,
                Range(0,3),
                Range::all(),
                Range::all(),
                Range::all());

                Conc_out_z_extract.SubData(Conc_out_z,
                Range(0,3),
                Range::all(),
                Range::all(),
                Range::all());

                Polair.Append(Conc_out_x_extract,file_x);
                Polair.Append(Conc_out_y_extract,file_y);
                Polair.Append(Conc_out_z_extract,file_z);*/
