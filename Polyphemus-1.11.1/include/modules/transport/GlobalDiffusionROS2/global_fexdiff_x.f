...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE global_FEXDIFF_X(Nx,Dcx,Dmx,DLconc,DLkx,rhox,DLr)
...
C------------------------------------------------------------------------
     ...
     s           * (DLconc(Ji)/rhox(Ji) - DLconc(Nx)/rhox(Nx))
         ELSEIF (Ji.EQ.Nx+1) THEN ! Boundary condition (right).
            Flux(Ji) = DLkx(1) / Dcx(1)
     s           * (DLconc(1)/rhox(1) - DLconc(Ji-1)/rhox(Ji-1))
         ELSE
            Flux(Ji) = DLkx(Ji) / Dcx(Ji)
