        ...
        if (Model.HasNumberConcentration_aer())
          {
            ...
            if (Model.HasNumberSurfaceEmission_aer(b))   
              {
                ...
                NumberSurfaceEmission_i
                  = Model.A3("NumberSurfaceEmission_aer_i")(emis_b,
                                                            Range::all(),
                                                            Range::all()).copy();
                NumberSurfaceEmission_f
                  = Model.A3("NumberSurfaceEmission_aer_f")(emis_b,
                                                            Range::all(),
                                                            Range::all()).copy();
              }
            else
              {
                NumberSurfaceEmission_i = 0;
                NumberSurfaceEmission_f = 0;
              }

            /*** Number Concentrations ***/

            Array<T, 3> NumberConcentration_aer_s(&Model.GetNumberConcentration_aer()
                                                (b, 0, 0, 0),
                                                shape(Model.GetNz(), Model.GetNy(),
                                                      Model.GetNx()));
            ...
            if (ParallelizedOnAerSpecies())
              ...
              Forward(Model.GetCurrentTime(), Model.GetDelta_t(),
                      ...
                      ModifiedCellCenterDistance_y,
                      ModifiedCellCenterDistance_z,
                      NumberSurfaceEmission_i, NumberSurfaceEmission_f,
                      Model.GetNx(), Model.GetNy(), Model.GetNz(),
