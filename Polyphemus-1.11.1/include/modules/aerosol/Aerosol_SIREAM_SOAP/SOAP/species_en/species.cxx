...
void creation_species( vector<species>& surrogate, vector<string> species_list_aer)
{
  ...
  BiA2D.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiA2D.name)
      BiA2D.soap_ind = i;

  surrogate.push_back(BiA2D);
  ...
  BiA1D.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiA1D.name)
      BiA1D.soap_ind = i;
  
  surrogate.push_back(BiA1D);
  ...
  BiA0D.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiA0D.name)
      BiA0D.soap_ind = i;
  
  surrogate.push_back(BiA0D);
  ...
  BiMT.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiMT.name)
      BiMT.soap_ind = i;
  
  surrogate.push_back(BiMT);
  ...
  BiPER.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiPER.name)
      BiPER.soap_ind = i;
  
  surrogate.push_back(BiPER);
  ...
  BiDER.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiDER.name)
      BiDER.soap_ind = i;

  surrogate.push_back(BiDER);
  ...
  BiMGA.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiMGA.name)
      BiMGA.soap_ind = i;
  
  surrogate.push_back(BiMGA);
  ...
  AnBlP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == AnBlP.name)
      AnBlP.soap_ind = i;
  
  surrogate.push_back(AnBlP);
  ...
  AnBmP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == AnBmP.name)
      AnBmP.soap_ind = i;
  
  surrogate.push_back(AnBmP);
  ...
  BiBlP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiBlP.name)
      BiBlP.soap_ind = i;
  
  surrogate.push_back(BiBlP);
  ...
  BiBmP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiBmP.name)
      BiBmP.soap_ind = i;
  
  surrogate.push_back(BiBmP);
  ...
  AnClP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == AnClP.name)
      AnClP.soap_ind = i;  

  surrogate.push_back(AnClP);
  ...
  BiNGA.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiNGA.name)
      BiNGA.soap_ind = i;
  
  surrogate.push_back(BiNGA);
  ...
  BiNIT3.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiNIT3.name)
      BiNIT3.soap_ind = i;

  surrogate.push_back(BiNIT3);
  ...
  BiNIT.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiNIT.name)
      BiNIT.soap_ind = i;  

  surrogate.push_back(BiNIT);
  ...
  POAlP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == POAlP.name)
      POAlP.soap_ind = i;

  surrogate.push_back(POAlP);
  ...
  POAmP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == POAmP.name)
      POAmP.soap_ind = i;

  surrogate.push_back(POAmP);
  ...
  POAhP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == POAhP.name)
      POAhP.soap_ind = i;

  surrogate.push_back(POAhP);
  ...
  SOAlP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == SOAlP.name)
      SOAlP.soap_ind = i;

  surrogate.push_back(SOAlP);
  ...
  SOAmP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == SOAmP.name)
      SOAmP.soap_ind = i;

  surrogate.push_back(SOAmP);
  ...
  SOAhP.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == SOAhP.name)
      SOAhP.soap_ind = i;

  surrogate.push_back(SOAhP);
  ...
  Monomer.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == Monomer.name)
      Monomer.soap_ind = i;
  
  surrogate.push_back(Monomer);
  ...
  Dimer.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == Dimer.name)
      Dimer.soap_ind = i;
  
  surrogate.push_back(Dimer);
  ...
  BiA3D.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == BiA3D.name)
      BiA3D.soap_ind = i;

  surrogate.push_back(BiA3D);
  ...
  orgNIT.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == orgNIT.name)
      orgNIT.soap_ind = i;
  
  surrogate.push_back(orgNIT);
  ...
  H2O.soap_ind = -1;
  for (int i = 0; i < nsp; ++i)
    if (species_list_aer[i].substr(1,-1) == H2O.name)
      H2O.soap_ind = i;
  
  surrogate.push_back(H2O);
