C-----------------------------------------------------------------------
C     Copyright (C) 2003-2017, ENPC - INRIA - EDF R&D
...
C     VERTICAL_INTERFACE: vertical interface height ([m]).
C
...
C     Set parameters
      ...
      ICOND = options_aer(2)
      INUCL = options_aer(3)
      ICLD = options_aer(4) !! Aqueous module
      IINCLD = options_aer(5) !! In-cloud scavenging
      IKELV = options_aer(6)
      IDENS = options_aer(7) !! Fixed density
      ICUT = options_aer(8)
      ...
      DO Jb=1,nbin_aer
         ...
         DO Jsp=1,nesp_aer
            ...
            if (DLconc_aer(Jb,Jsp) .GT. TINYM) then
               ...
               conc_tot(jb) = conc_tot(jb) + ZA(i)
            else
               ZA(i) = 0.d0
            endif
         ENDDO
         IF ((conc_tot(jb) .LE. TINYM).AND.(INUM.EQ.1)) then
            ZNA(Jb) = 0.d0
         ENDIF
      ENDDO
...
C     (YK:2018/07/28)
            if (inum .ne. 1) then
C           Compute aerosol volume in microm^3
C           = (microg / m^3) / (microg / microm^3)
               if (rho_aero(jb) .le. 0.d0) then
                  write(*,*) "Wrong aerosol density:", jb, rho_aero(jb)
                  stop
               else
                  volume = conc_tot(jb) / rho_aero(jb)
               endif

C     Compute number concentration.
               if (DSF(jb) .le. 0.d0) then
                  write(*,*) "Wrong aerosol diameter:", jb, DSF(jb)
               else
                  ZNA(jb) = (6.d0 * volume) / ((DSF(jb) ** 3.d0) * PI)
               endif
     ...
     &              LMD,Jb,rho_aero(Jb))
            ...
            IF (DLnum_conc_aer(Jb).GT.0.D0) THEN
               dold(Jb) = (conc_tot(jb)/DLnum_conc_aer(Jb)
     &              /cst_PI6/rho_aero(Jb))**cst_FRAC3
            ELSE
      ...
      DO Jb=1,nbin_aer
         DO Jsp=1,nesp_aer
            ...
            IF (isNaN(DLconc_aer(Jb,Jsp))) then
               ...
               STOP "aerosol.f :  aerosol concentration is NaN"
            endif
         ENDDO
      ENDDO
...
!     Compute particle sizes
       do Jb = 1, nbin_aer
          conc_tot(jb) = 0.d0
          DO Jsp = 1, Nesp_aer-1
             conc_tot(jb) = conc_tot(jb) + DLconc_aer(Jb, Jsp)
          ENDDO
          IF (IDENS .EQ. 1) THEN
             CALL compute_density(nbin_aer,nesp_aer, nesp_aer,TINYM,
     &            DLconc_aer,
     &            LMD,Jb,rho_aero(Jb))
          ENDIF
          IF (DLnum_conc_aer(Jb).GT.0.D0) THEN
             DSF2(Jb) = (conc_tot(jb)/DLnum_conc_aer(Jb)
     &            /cst_PI6/rho_aero(Jb))**cst_FRAC3
          ELSE
             DSF2(Jb) = DSQRT(DBF(Jb) * DBF(Jb+1))
          ENDIF
