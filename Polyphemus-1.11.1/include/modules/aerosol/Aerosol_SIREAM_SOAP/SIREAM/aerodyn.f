     ...
     s           psoap_config,psurrogate,ISOAPDYN)
         ...
         DO js = 1,nbin_aer
            DO jesp = 1,nesp_aer-1
               if (isNaN(q(IQ(jesp,js)))) then
                  print *, js, jesp, "q(IQ) NaN apres coag/processaero"
                  stop
               endif
            ENDDO
         ENDDO
...
C     update variables for dispersion equation
     ...
     &     ZA(NGAS+(nesp_aer-1)*nbin_aer+1),
     &     ZA(NGAS+(nesp_aer-1)*nbin_aer+2),
     &     ZA(NGAS+(nesp_aer-1)*nbin_aer+3),
     &     ZA(NGAS+(nesp_aer-1)*nbin_aer+4),
     &     ZA(NGAS+(nesp_aer-1)*nbin_aer+5)
