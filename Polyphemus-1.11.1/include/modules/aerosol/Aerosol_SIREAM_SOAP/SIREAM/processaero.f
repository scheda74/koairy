...
C-----------------------------------------------------------------------
     ...
     s     psoap_config, psurrogate,ISOAPDYN)
...
C     2017/09/19: Adapt for implementation of SOAP (Youngseob Kim, CEREA).
C
C------------------------------------------------------------------------
C
C     -- AUTHOR(S)
...
C------------------------------------------------------------------------
      ...
      integer iteration,ib,js,jesp
      DOUBLE PRECISION ionic,proton,lwc
      double precision inorg_total,inorg_bin(nbin_aer)
      integer ISOAPDYN
...
C     so-called bulk approach
     ...
     s           psoap_config,psurrogate,
     s           ISOAPDYN,ionic,proton,lwc,liquid)

C     solve SOA dynamically
            IF (ISOAPDYN.EQ.1) then
! Redistriution of LWC into aerosol bins.
! !!! Temporary measure !!!
! LWC_bin = LWC_total * (inorganics_bin / inorganics_total)
! YK & KS 2014/11/28
               inorg_total = 0.D0
               inorg_bin = 0.D0
               do jesp=ENa,ECl
                  do js=1,nbin_aer !! ICUT2
                     inorg_total = inorg_total + q(IQ(jesp,js))
                     inorg_bin(js) = inorg_bin(js) + q(IQ(jesp,js))
                  enddo
               enddo

               do js=1,nbin_aer
                  if (inorg_total .gt. 0.D0) then
                     q(IQ(EH2O,js)) = lwc * inorg_bin(js) / inorg_total
                  else
                     q(IQ(EH2O,js)) = 0.d0
                  end if
               enddo

               !! Timestep for SOAP_dyn is not DT2 but DT.
               !! DT2 can be used in the dynamic model for inorganic aerosols.
               CALL soap_dyn(nesp_aer, nbin_aer, rh,
     &              ionic, proton, lwc, liquid,
     &              temp, psoap_config,psurrogate, DT,
     &              DSD, neq, q, iq)
