...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE PROCESSAERO(neq,nbin_aer,q,iq,couples_coag,
...
C     solve sulfate dynamically
            IF (ISULFCOND.EQ.1) CALL SULFDYN(neq,nbin_aer,q,iq,QT,
     &           XSF,MSF,DSF,XSD,MSD,DSD)

C     solve equilibrium with
C     so-called bulk approach
            IF (ICUT2.GT.0) CALL BULKEQUI(neq,nbin_aer,q,iq,QT,
     ...
     s        ICG2.EQ.0) THEN
         ...
         ElSEIF (KDSLV2.EQ.1) THEN
            ...
            CALL ETRCONC(neq,nbin_aer,q1,q,iq,couples_coag,
     s      first_index_coag,second_index_coag,
     s      coefficient_coag,QT,XSF,MSF,DSF,XSD,MSD,DSD)
         ...
         ELSEIF (KDSLV2.EQ.2) THEN
            ...
            CALL ROS2CONC(neq,nbin_aer,q1,q,iq,couples_coag,
     s      first_index_coag,second_index_coag,
     s      coefficient_coag,QT,XSF,MSF,DSF,XSD,MSD,DSD)
         ...
         ELSEIF (KDSLV2.EQ.3) THEN
            ...
            CALL EBICONC(neq,nbin_aer,q1,q,iq,QT,
     s      XSF,MSF,DSF,XSD,MSD,DSD)
         ENDIF
...
C     check the need to call ADAPTSTEP (not for the last timestep)
         IF (TIN2.LT.TOUT2 ) THEN
            CALL ADAPTSTEP(neq,nbin_aer,q1,q,iq)
         ENDIF
