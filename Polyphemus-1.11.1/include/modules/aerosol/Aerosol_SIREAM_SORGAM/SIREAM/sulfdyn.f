...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE SULFDYN(neq,nbin_aer,q,iq,QT,XSF,MSF,DSF,XSD,MSD,DSD)
...
C     Compute diameter with thermo or gerber formula
      IF (ITHRM.EQ.0) THEN
         CALL EQPART(nbin_aer,1,nbin_aer,neq,q,iq,QT,
     s        XSF,MSF,DSF,XSD,MSD,DSD,
     s        QINT,QGEQ,VSW,MSW,DSW,RHOW)
      ELSEIF (ITHRM.EQ.1) THEN
         CALL FASTDIAM(nbin_aer,1,nbin_aer,neq,q,iq,QT,
