...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE EQPART(nbin_aer,is1,is2,neq,q,iq,QT,
...
C------------------------------------------------------------------------
      ...
      INTEGER is1,is2,neq,nbin_aer
      DOUBLE PRECISION q(neq)

      INTEGER js,jj,jesp,iq(NEXT,nbin_aer)
      DOUBLE PRECISION qext(NEXT)

      DOUBLE PRECISION QGEQTMPA(NEXT),QINTMPA(NINTIS)
     ...
     &        QT(js).GT.TINYM ) THEN
            ...
            CALL STEP( q(js),   ! num aero conc (#part.m-3)
