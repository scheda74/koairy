...
C     QINTI : internal  solid aerosol concentration ([µg.m-3]).
...
C     -- OUTPUT VARIABLES
C
C     QGEQI  : gas-phase concentration at the aerosol surface ([µg.m-3].
...
C     56 for nano3 and nh4no3 reactions
     ...
     &     *ckvi(ENH3)*ckvi(ECl)
      ...
      sat1=qgbki(ENH3)*qgbki(ENO3) ! (µg.m-3)2
      sat2=qgbki(ENH3)*qgbki(ECl) ! (µg.m-3)2

      sat3a=qgbki(ECl)          ! µg.m-3
      sat3b=rk3*qgbki(ENO3)     ! µg.m-3
