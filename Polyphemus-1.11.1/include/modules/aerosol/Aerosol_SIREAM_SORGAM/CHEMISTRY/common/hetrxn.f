...
C     If we are in a cloud, then reactions on droplet surface for NO3 and HO2.
         IF ((ICLOUD.EQ.1).and.(i.eq.4)) THEN
            Ndroplet = lwctmp
     &           / (RHOwater * 1.d3 !kg.m-3 -> g.m-3
     &           * (1.d0 / 6.d0) * pi * avdiammeter ** 3.d0)
            A = PI*(avdiammeter**2.d0)*Ndroplet !surface * nb_droplet
            IF(Gamma(i).ne.0.d0) then
               ktot(i) = ktot(i) + (((avdiammeter/(2.d0*DIFF))+4.d0/
     &           (vi*Gamma(i)))**(-1.d0))*A ! rate constant for each rxn
            ENDIF
         ENDIF
      enddo
