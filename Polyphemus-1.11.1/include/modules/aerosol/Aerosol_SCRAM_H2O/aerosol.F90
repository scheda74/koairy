...
!!     Author(s): Debry Edouard
!!
!!     This file is part of the Size Resolved Aerosol Model (SIREAM), a
...
!!------------------------------------------------------------------------
      ...
      DOUBLE PRECISION initial_time,delta_t
      ...
      !!out put value
      DOUBLE PRECISION, intent(out) :: Wet_Deposition(nesp)
...
!!    aInitialization     
	...
	do g=1,N_groups
	  do k=1,2
	    ...
	    !print*,f,g,k,discretization_composition(f,g,k)
	    id=id+1
	  enddo
	enddo
      enddo
      !print*,"finish reading composition_bounds"
...
! 	call check_nan_inf(3)
	...
	enddo
	!print*,"compute_average_diameter"
	call compute_average_diameter()
! 	call check_nan_inf(4)
	!print*,"compute_wet_mass_diameter"
         ...
         IF (cloud_water.GE.LWCmin) THEN
	       ...
	       !compute qscav_gas ; qscav_aer ; qscav_num
            ...
            ELSE IF(aqueous_module.EQ.2) THEN ! Use simple aqueous module.
               CALL simple_aqueous_module(N_gas,N_aerosol,&
               N_size,N_sizebin,nesp_cloud_interact,&
               density_aer_bin,fixed_density,&
               diam_bound,cell_diam_av,log_bound,&
               concentration_gas_loc,concentration_mass,&
               ...
               cloud_species_interact,concentration_index,&
               List_species,nesp_isorropia,nesp_aec,nesp_pankow,&
               nesp_pom,section_pass,redistribution_method,with_fixed_density)
...
! 		concentration_gas(jesp)*(concentration_mass(j,jesp)/total_aero_mass(jesp))
! 	      ENDDO
	      print*,"wanning! gas<0 species:",jesp,concentration_gas(jesp)
	    ...
	    ENDIF
         ENDDO
!          write(unit=10,FMT=*),"total_aero_mass",tot_tot
!          CLOSE(10)
         DO j=1,nbin_aer
            DO jesp=1,nesp_aer
		...
		stop
            ...
            ENDDO
         ENDDO
!          print*,"last DU",DLconc_aer(121,1)
	 total_nb=0.d0
!!     Number concentration
         IF(with_number.EQ.1) THEN
            DO j=1,nbin_aer
               ...
               total_nb=total_nb+concentration_number(j)
            ENDDO
!             if(total_nb*tot_tot.eq.0.d0.and.total_nb.ne.tot_tot) then
! 	      print*,"total_nb",total_nb,"total_aer",tot_tot
! 	    endif
         ENDIF
