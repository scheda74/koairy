...
!------------------------------------------------------------------------   
	...
	do s= 1, (N_aerosol-1)
	  ...
	  if(mass_density_aer(s).gt.0.d0) then
	...
	do s= 1, (N_aerosol-1)
	  ...
	  if(mass_density_aer(s).gt.0.d0) then
	    volum_sizebin(k)=volum_sizebin(k)+&
	      concentration_mass(j,jesp)/mass_density_aer(s)
	  endif
	enddo
      enddo
...
!------------------------------------------------------------------------  
	...
	if(mass_density_aer(s).gt.0.d0) then
	  ...
	  print*,"Wrong mass_density_aer",s,mass_density_aer(s)
	endif
      enddo
...
!------------------------------------------------------------------------  
    ...
    do k=1, N_size
      do s = 1, (N_aerosol-1)
        jesp=List_species(s)
        if(mass_density_aer(s).gt.0.d0) then
           subrho(k) = subrho(k) + concentration_mass(k,jesp)/mass_density_aer(s)
        endif
