...
! 	    enddo
	 ...
	 elseif(wet_diam_estimation.eq.1) then!with fastdiam
	   ...
	   qext(EH2O)=DMAX1(wet_v(j)-vad,0.D0)*mass_density_aer(EH2O) ! water conc (µg.m-3)
	   c_mass(j,EH2O)=qext(EH2O)
	   wet_m(j)=(qti+qext(EH2O))/c_number(j) ! single wet mass (µg)
...
!     56 for nano3 and nh4no3 reactions
    ...
    ! YK
    call equi_const(Temperature, XK3, XK4, XK6, XK8, XK9, XK10)
...
!------------------------------------------------------------------------
    ...
    REAL frh,ftempk
    double precision fdeltalwc,forganion
...
!       if(IsNaN(flwc*0.d0)) print*,8.1,flwc,"AEC_DRV"
!       if(IsNaN(fprotonconc*0.d0)) print*,8.2,fprotonconc,"AEC_DRV"
      ! microg/m3(=micromol/m3) / microg/m3 = micromol/microg = mole / g
    endif
    if(lwc .gt. 1.d06) print*,"lwc>1e6",lwc,"AEC_DRV -1"
...
!     concentrations in microg.m-3
    do i = 1,nesp_aec
      ...
      gasorg(i) = REAL(gas(j))
!       if(IsNaN(partorg(i)*0.d0)) print*,'nan a',j,partorg(i),aero(j),"AEC_DRV"
!       if(IsNaN(gasorg(i)*0.d0)) print*,'nan g',j,gasorg(i),gas(j),"AEC_DRV"
      worg(i) = partorg(i) + gasorg(i)
!        print*,"input partorg",i,partorg(i)
!        print*,"input gasorg",i,gasorg(i)
    enddo
...
!       if(IsNaN(worg_poa*0.d0)) print*,10.1,worg_poa,i,"AEC_DRV"
!       if(IsNaN(mwaom_mix*0.d0)) print*,10.2,mwaom_mix,i,"AEC_DRV"
    enddo
...
!     print*,"worg:",worg
!     print*,"gasorg:",gasorg
!     print*,"partorg:",partorg
!     print*,"fmwaom_mix",fmwaom_mix
!     print*,"flwc",flwc,"fprotonconc",fprotonconc
!     print*,"forganion",forganion
!     print*,"fdeltalwc",fdeltalwc
!     print*,"flag",flag,"with_oligomerization",with_oligomerization
!     print*,"qsatref_loc",qsatref_loc
!     print*,"tsatref_loc",tsatref_loc
!     print*,"kpartref_loc",kpartref_loc
...
!     Give back concentrations
	...
	gas(j) = DBLE(gasorg(i))
! 	print*,'aec_drv out',j,aero(j),gas(j)
! 	if(IsNaN(gas(j)*0.d0)) print*,14.1,j,gas(j),"AEC_DRV"
! 	if(IsNaN(aero(j)*0.d0)) print*,14.2,j,aero(j),"AEC_DRV"
! 	print*,"after partorg",i,partorg(i)
! 	print*,"after gasorg",i,gasorg(i)
    enddo
...
! 	print*,"input",idx,"aero",aero(idx),"gas",gas(idx)
    enddo
!     conversion unit for isorropia needed in mol.m-3
    do i=1,nesp_isorropia
	idx = isorropia_species(i)
	wi(i) = wi(i) / molecular_weight_aer(idx) ! microg.m-3 / microg.mol-1 = mol.m-3
! 	print*,"gas+aero",idx,wi(i), molecular_weight_aer(idx)
    enddo
!     call isorropia fortran routine
!     print*,"ISOROPIA Input:"
...
! 	print*,"output",idx,"aero",aero(idx),"gas",gas(idx)
    enddo
