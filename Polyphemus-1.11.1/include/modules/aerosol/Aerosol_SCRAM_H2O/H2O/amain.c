...
void amain (double *aero, double *aeros, double *deltaLWC2, int* ioligo)
{
  ...
  if (LWC > LWC_ZERO) {
	...
	for (j = 0; j < NK[i]; j++) {
	  ...
	  /*if(aero[ieq]>HUGE)
	  {
	    printf("aero[%d]=%e , LWC=%e acHP=%e totA[%d] %e wrong 1 \n",
		   ieq,aero[ieq],LWC,acHP,i,totA[i]);
	    printf("aero[1]=%e aero[2]=%e Keff[1]=%e Keff[2]=%e Keff[3]=%e MW[%d]=%e\n",
		   aero[1],aero[2],Keff[jeq],Keff[jeq+1],Keff[jeq+2],aidx[ieq],MW[aidx[ieq]]);
	  }*/
	  ieq ++;
	}                         /* end for j */
      }                           /* end if aqueous */
