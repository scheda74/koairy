...
float fmin1(float x[])
{
	...
	for (sum=0.0,i=1;i<=nn1;i++)
	{
	  if (x[i]<1.0e-40) x[i]=0.0;
	  sum += SQR(x[i]);
	}
	return 0.5*sum;
}
...
double fmin_double(double x[])
{
	...
	for (sum=0.0,i=1;i<=nn;i++)
	  {
	    if (fvec_double[i]<1.0e-40) fvec_double[i]=0.0;
	    sum += SQR(fvec_double[i]);
	  }
	return 0.5*sum;
}
...
double fmin1_double(double x[])
{
	...
	for (sum=0.0,i=1;i<=nn1;i++)
	  {
	    if (fvec1_double[i]<1.0e-40) fvec1_double[i]=0.0;
	    sum += SQR(fvec1_double[i]);
	  }
	return 0.5*sum;
}
	...
	void (*vecfunc)(int, double [], double []))
{
	...
	for (its=1;its<=MAXITS;its++) {
		fdjac_double(n,x,fvec_double,fjac_double,vecfunc);/*?fjac_double changed*/
/*		printf("its: %d fdjac_double: x[1]:%e x[2]:%e x[3]%e EPS:%e \n",its,x[1],x[2],x[3],EPS);*/
		for (i=1;i<=n;i++) {
		...
		fold=f;
		for (i=1;i<=n;i++) p_double[i] = -fvec_double[i];/*change sign*/
		ludcmp_double(fjac_double,n,indx,&d);/*?fjac_double changed*/
		if (anrerrflag == 1) FREEDRETURN
		lubksb_double(fjac_double,n,indx,p_double);/*?p_double changed*/
		lnsrch_double(n,xold_double,fold,g_double,p_double,x,&f,stpmax,check,fmin_double);/*??*/
/*		printf("lnsrch_double: x[1]:%e x[2]:%e x[3]%e \n",x[1],x[2],x[3]);
		printf("lnsrch_double: p[1]:%e p[2]:%e p[3]:%e \n",p_double[1],p_double[2],p_double[3]);*/
		test=0.0;
		for (i=1;i<=n;i++)
		  if (fabs(fvec_double[i]) > test) test=fabs(fvec_double[i]);
		if (test < TOLF) {
