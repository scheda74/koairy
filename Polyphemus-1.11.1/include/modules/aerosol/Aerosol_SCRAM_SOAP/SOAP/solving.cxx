          ...
          if (LWCtot>config.LWClimit)
            {
              ...
              for (i=0;i<n;i++)
                if (surrogate[i].hydrophilic and (surrogate[i].Aaq_bins(b)>1.0e-5 or surrogate[i].Aaq_bins_init(b)>1.0e-5) and config.iHp!=i)
                  {
		    ...
		    if (abs(surrogate[i].Aaq_bins_init(b)
			    -surrogate[i].Aaq_bins(b))
			/surrogate[i].Aaq_bins(b)>config.relative_precision/nh_aq and index==config.max_iter-1)
		      cout << surrogate[i].name << " " << abs(surrogate[i].Aaq_bins_init(b)
					      -surrogate[i].Aaq_bins(b))
			/surrogate[i].Aaq_bins(b) << " " << surrogate[i].Aaq_bins_init(b) << endl;*/
		  }
	      if (index == config.max_iter-1)
		{
		  cout << nh_aq << endl;
		  cout << chp << endl;
		}
  ...
  if (error_org>config.relative_precision/nh_org or error_aq>config.relative_precision/nh_aq)
    {
      ...
      cout << "uncoupled" << endl;
      cout << "error org" << error_org*nh_org << " " << nh_org << endl;
      cout << "error aq" << error_aq*nh_aq << " " << nh_aq << endl;
