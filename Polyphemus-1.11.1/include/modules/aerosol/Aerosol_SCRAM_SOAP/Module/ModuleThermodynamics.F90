...
!------------------------------------------------------------------------
	...
	qti=qti+c_mass(j,jesp)     ! µg.m-3
      ...
      if (wet_d(j) .lt. 1.d-3) then
         write(*,*) "bulkequi_inorg: too small wet_diameter",wet_d(j)
         wet_d(j) = 1.d-3
      endif !! YK & KS

    enddo

    if(total_IH*total_water.gt.0.d0) &
...
! aero in microg/m3
  ...
  subroutine ISOROPIA_DRV(nesp_aer,&
          aero, gas, organion, watorg, ionic, proton, lwc, Relative_Humidity, Temperature, &
          liquid)
