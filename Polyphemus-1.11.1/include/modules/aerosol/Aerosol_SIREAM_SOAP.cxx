...
#ifndef POLYPHEMUS_FILE_MODULES_AEROSOL_AEROSOL_SIREAM_SOAP_CXX

#include "Aerosol_SIREAM_SOAP.hxx"
...
namespace Polyphemus
{
  ...
  //! Default constructor.
  template<class T>
  Aerosol_SIREAM_SOAP<T>::Aerosol_SIREAM_SOAP()
  {
  ...
  */
  ...
  void Aerosol_SIREAM_SOAP<T>::Init(ClassModel& Model)
  {
    if (Model.GetNs() != Ns)
	...
	if (option_process_aer["collect_wet_flux"])
	  {
        ...
        if (aqueous_module == "no")
          throw string("Warning: activate an aqueous module ") +
            ...
            string("See aerosol.f and Aerosol_SIREAM.cxx");
      }
  ...
  template<class T>
  template<class ClassModel>
  void Aerosol_SIREAM_SOAP<T>::DisplayConfiguration(ClassModel& Model)
  {
  ...
  */
  template<class T>
  template<class ClassModel>
  void Aerosol_SIREAM_SOAP<T>::Forward(ClassModel& Model)
  {
  ...
  */
  template<class T>
  template<class ClassModel>
  void Aerosol_SIREAM_SOAP<T>::Forward_aer(ClassModel& Model)
  {
    Date date_i = Model.GetCurrentDate();
    Date date_f = Model.GetNextDate();
  ...
  template<class T>
  void Aerosol_SIREAM_SOAP<T>::Forward(T current_time,
  ...
  {
    throw string("'Aerosol_SIREAM_H20_0D' requires an aerosol aware model.");
  }
...
#pragma omp parallel for num_threads(Nthreads_openmp)	\
  ...
  */
  template<class T>
  void Aerosol_SIREAM_SOAP<T>::Forward_aer(T current_time,
					  Data<T, 3>& SpecificHumidity_i,
					  Data<T, 3>& Temperature_i,
					  Data<T, 3>& Pressure_i,
					  T delta_t,
...
#pragma omp parallel for num_threads(Nthreads_openmp)	\
                ...
                for (s = 0; s < Ns_aer ; s++)
                  {
                    ...
                    // Rank of indices must match the one defined in
                    // ScatterSlice_x_MPI.
                    OrdWetDepositionFlux_aer(i, b, j, s) += InCloudWetDepositionFlux_aer2D(s, b);
#else
                    InCloudWetDepositionFlux_aer(s, b, j, i) += InCloudWetDepositionFlux_aer2D(s, b);
#endif
                  }
              }
  ...
  {
    _aerosol_0d(&Ns, &Ns_isorropia, &Ns_aec, &Ns_pankow,
                ...
                &pankow_species.front(), &poa_species.front(), &md_species,
                &bc_species,
  ...
  */
  template<class T>
  bool Aerosol_SIREAM_SOAP<T>::IsRequired(string field)
  {
  ...
  */
  template<class T>
  bool Aerosol_SIREAM_SOAP<T>::IsComputed(string field)
  {
  ...
  template<class T>
  void Aerosol_SIREAM_SOAP<T>::FogSettling(Array<T, 1>& LiquidWaterContent,
...
#define POLYPHEMUS_FILE_MODULES_AEROSOL_AEROSOL_SIREAM_SOAP_CXX
#endif
