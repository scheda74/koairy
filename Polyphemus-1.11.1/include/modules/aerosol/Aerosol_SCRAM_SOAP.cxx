...
#ifndef POLYPHEMUS_FILE_MODULES_AEROSOL_AEROSOL_SCRAM_SOAP_CXX

#include "Aerosol_SCRAM_SOAP.hxx"
...
namespace Polyphemus
{
  ...
  //! Default constructor.
  template<class T>
  Aerosol_SCRAM_SOAP<T>::Aerosol_SCRAM_SOAP()
  {
  ...
  */
  ...
  void Aerosol_SCRAM_SOAP<T>::Init(ClassModel& Model)
  {
    if (Model.GetNs() != Ns)
    ...
    config.PeekValue("With_external_composition",
		     option_process_aer["with_external_composition"]);
    ...
    // Lwc cloud threshold.
    config.PeekValue("Relative_error_precision", relative_error);
    
    // SOA computation approach.
    config.PeekValue("SOA_computation", soa_computation);
    soa_computation = lower_case(soa_computation);

    // Diffusion coefficient in the organic phase.
    config.PeekValue("Diffusion_coefficient_organic", dorg);
    soap_config.dorg = dorg;

    // Number of aerosol layers.
    config.PeekValue("Number_of_aerosol_layers", nlayer);
    soap_config.nlayer = nlayer;

    // Minimum timestep for aerosols (in s).
    config.PeekValue("Minimum_timestep_for_aerosols", dtaeromin);
    soap_config.deltatmin = dtaeromin;

    // Criterium to distinguish cases under equilibrium
    // characteristic time lower than this time (in s).
    config.PeekValue("Equilibrium_time", tequilibrium);
    soap_config.tequilibrium = tequilibrium;

    // If aqueous and organic phases are coupled
    config.PeekValue("Coupled_phases", coupled_phases);
    coupled_phases = lower_case(coupled_phases);
    if (coupled_phases == "yes")
      soap_config.coupled_phases = true;
    else if (coupled_phases == "no")
      soap_config.coupled_phases = false;                           
    else
      throw string("bad string \"") + coupled_phases +
        string("\" for Coupled_phases option,\n") +
        string("possibilities are yes or no.");
    ...
    if (option_process_aer["with_kelvin_effect"])
      {
        options_aer(5) = 1;
        soap_config.compute_kelvin_effect = true;
      }
    else
      {
        options_aer(5) = 0;
        soap_config.compute_kelvin_effect = false;
      }
...
//       options_aer(9) = 3;
    ...
    /* Options for SOAP */
    if (soa_computation == "dynamic")
      {
        options_aer(14) = 1;
        soap_config.equilibrium = false;
      }
    else if (soa_computation == "equilibrium")
      {
        options_aer(14) = 0;
        soap_config.equilibrium = true;
      }
    else
      throw string("bad string \"") + soa_computation +
                                       string("\" for SOA computation option,\n") +
                                       string("possibilities are dynamic or equilibrium.");
    ...
    else if (Thermodynamic_model == "aiomfac")
      {
        soap_config.activity_model = "unifac";
        soap_config.compute_long_and_medium_range_interactions = true;
      }
    else
      throw string("Bad option \"") + Thermodynamic_model +
                                       "\" given to the organic thermodynamic model,\n"
                                       "possibilities are \"unifac\", \"aiomfac\" or \"ideal\".";
    ...
    //     string(" configuration file."); (16/07/2018 KS: removed)
    // if (option_process_aer["with_number_concentration"] == 0
  ...
  template<class T>
  template<class ClassModel>
  void Aerosol_SCRAM_SOAP<T>::DisplayConfiguration(ClassModel& Model)
  {
    species_list = Model.GetSpeciesList();
    species_list_aer = Model.GetSpeciesList_aer();
    Ncomposition_aer=Model.GetNcomposition_aer();
    ...
    if (option_process_aer["with_external_composition"])
      cout << "Module: with external composition" << endl;
    ...
    cout << "Module: SOA computation approach:"
         << soa_computation << endl;
  ...
  */
  template<class T>
  template<class ClassModel>
  void Aerosol_SCRAM_SOAP<T>::Forward(ClassModel& Model)
  {
  ...
  */
  template<class T>
  template<class ClassModel>
  void Aerosol_SCRAM_SOAP<T>::Forward_aer(ClassModel& Model)
  {
    Date date_i = Model.GetCurrentDate();
    Date date_f = Model.GetNextDate();
  ...
  */
  template<class T>
  void Aerosol_SCRAM_SOAP<T>::Forward(T current_time,
...
#pragma omp parallel for num_threads(Nthreads_openmp)	\
  ...
  */
  template<class T>
  void Aerosol_SCRAM_SOAP<T>::Forward_aer(T current_time,
					  Data<T, 3>& SpecificHumidity_i,
					  Data<T, 3>& Temperature_i,
					  Data<T, 3>& Pressure_i,
					  T delta_t,
...
//#else
     ...
     __zaerosolscram_MOD_aerosol(&Ns, &Ns_isorropia, &Ns_aec, &Ns_pankow,
	     ...
	     Ncoefficient.data(),index_first.data(),index_second.data(),coefficient.data(),
 	     ...
 	     incloudwetdepositionfluxnumber_aer.data(),&conserving_mass_tolerance,
                                 ...
                                 &soap_config, &surrogate, &dtaeromin,
                                 &relative_error);
  ...
  */
  template<class T>
  bool Aerosol_SCRAM_SOAP<T>::IsRequired(string field)
  {
  ...
  */
  template<class T>
  bool Aerosol_SCRAM_SOAP<T>::IsComputed(string field)
  {
  ...
  template<class T>
  void Aerosol_SCRAM_SOAP<T>::FogSettling(Array<T, 1>& LiquidWaterContent,
  ...
  template<class T>
  void Aerosol_SCRAM_SOAP<T>::Read_Coagulation_Coefficient(const string &input_file)
  {
...
#define POLYPHEMUS_FILE_MODULES_AEROSOL_AEROSOL_SCRAM_SOAP_CXX
#endif
