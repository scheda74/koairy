...
C     Livermore National Laboratory (UC LLNL) under contract
C     no. W-7405-ENG-48 (Contract 48) between the U.S. Department of
C     Energy (DOE) and The Regents of the University of California
C     (University) for the operation of UC LLNL. The rights of the
C     Federal Government are reserved under Contract 48 subject to the
C     restrictions agreed upon by the DOE and University as allowed
C     under DOE Acquisition Letter 97-1.
C
C
C     The VODE package has been declared to be in the Public Domain.
C
C
C     DISCLAIMER
C
C     This work was prepared as an account of work sponsored by an
...
C     privately-owned rights. Reference herein to any specific
C     commercial products, process, or service by trade name, trademark,
C     manufacturer or otherwise does not necessarily constitute or imply
C     its endorsement, recommendation, or favoring by the United States
C     Government or the University of California. The views and opinions
C     of authors expressed herein do not necessarily state or reflect
C     those of the United States Government or the University of
C     California, and shall not be used for advertising or product
C     endorsement purposes.
C-----------------------------------------------------------------------
...
!$OMP THREADPRIVATE(MORD, MXHNL0, MXSTP0)
      SAVE ZERO, ONE, TWO, FOUR, PT2, HUN
!$OMP THREADPRIVATE(ZERO, ONE, TWO, FOUR, PT2, HUN)
...
C-----------------------------------------------------------------------
 ...
 621        EWTI = RWORK(LEWT+I-1)
            ...
            GO TO 700
 622        CONTINUE
            MSG='DVODE--TOUT(=R1) too closetoT(=R2)tostartintegration'
...
!$OMP THREADPRIVATE(ADDON, BIAS1, BIAS2, BIAS3)
!$OMP THREADPRIVATE(ETACF, ETAMIN, ETAMX1, ETAMX2, ETAMX3, ETAMXF, ETAQ)
!$OMP THREADPRIVATE(ETAQM1, KFC, KFH, MXNCF, ONEPSM, THRESH, ONE, ZERO)
C-----------------------------------------------------------------------
      COMMON /DVOD01/ ACNRM, CCMXJ, CONP, CRATE, DRC, EL(13),
     1     ETA, ETAMAX, H, HMIN, HMXI, HNEW, HSCAL, PRL1,
     2     RC, RL1, TAU(13), TQ(5), TN, UROUND,
...
C     Compute ratio of new H to current H at the current order. ------------
                     ...
                     IF ((KFLAG.LE.-2).AND.(ETA.GT.ETAMXF))
     &        ETA=ETAMXF
                     GO TO 150
C-----------------------------------------------------------------------
...
C     Compute ratio of new H to current H at the current order less one. ---
                        ...
                        ETAQM1=ONE/((BIAS1*DDN)**(ONE/(FLOTL-ONE))
     &        +ADDON)
 570                    ETAQP1 = ZERO
...
C     Test tentative new H against THRESH, ETAMAX, and HMXI, then exit. ----
 630                       IF (ETA .LT. THRESH .OR. ETAMAX .EQ. ONE)
     &                     GO TO 640
                           ETA = MIN(ETA,ETAMAX)
...
!$OMP THREADPRIVATE(CORTES, ONE, SIX, TWO, ZERO)
C
      COMMON /DVOD01/ ACNRM, CCMXJ, CONP, CRATE, DRC, EL(13),
     1     ETA, ETAMAX, H, HMIN, HMXI, HNEW, HSCAL, PRL1,
     2     RC, RL1, TAU(13), TQ(5), TN, UROUND,
...
!$OMP THREADPRIVATE(CCMAX, CRDOWN, MAXCOR, MSBP, RDIV, ONE, TWO, ZERO)
C
      COMMON /DVOD01/ ACNRM, CCMXJ, CONP, CRATE, DRC, EL(13),
     1     ETA, ETAMAX, H, HMIN, HMXI, HNEW, HSCAL, PRL1,
     2     RC, RL1, TAU(13), TQ(5), TN, UROUND,
...
!$OMP THREADPRIVATE(ONE, PT1, THOU, ZERO)
C-----------------------------------------------------------------------
      COMMON /DVOD01/ ACNRM, CCMXJ, CONP, CRATE, DRC, EL(13),
     1     ETA, ETAMAX, H, HMIN, HMXI, HNEW, HSCAL, PRL1,
     2     RC, RL1, TAU(13), TQ(5), TN, UROUND,
...
C     If MITER = 2, make N calls to F to approximate the Jacobian. ---------
            ...
            R0 = THOU*ABS(H)*UROUND*DBLE(N)*FAC
...
C     If JOK = -1 and MITER = 4, call JAC to evaluate Jacobian. ------------
                     ...
                     DO 410 I = 1,LENP
 410                    WM(I+2) = ZERO
                        CALL JAC (N, TN, Y, ML, MU, WM(ML3),
     &                  MEBAND, RPAR, IPAR)
                        IF (JSV .EQ. 1)
     1                       CALL DACOPY (MBAND, N, WM(ML3),
     &                       MEBAND, WM(LOCJS), MBAND)
                     ENDIF
C
...
!$OMP THREADPRIVATE(LENRV1, LENIV1, LENRV2, LENIV2)
C
