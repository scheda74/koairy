...
C     Modification for the aerosol number concentrations (Stéphanie Deschamps).
C
C-----------------------------------------------------------------------
C
C     -- AUTHOR(S)
...
C     Concentration updated with the pH.
      ...
      double precision foa(Nesp_pom + nesp_aec + nesp_pankow, nbin_aer)
      ...
      integer ifirstact
      integer isect, isp, i, istep, ii, nistep, j, jj
      ...
      double precision dold(nbin_aer)
      double precision qscav_num(nbin_aer)
      double precision fixed_diameter(nbin_aer)
...
C     Particle phase concentration in \mu g.m-3.
      do i = 1, nbin_aer
         ...
         do j = 1, Nesp_pom
            jj = poa_species(j)
            aerosol(i, nao) =
     $           aerosol(i, nao) + ZA(ii + (jj-1) * nbin_aer)
            foa(j, i) = ZA(ii + (jj-1) * nbin_aer)
            ftot = ftot + ZA(ii + (jj-1) * nbin_aer)
         enddo

         DO j = 1,nesp_aec
            jj = aec_species(j)
            aerosol(i,nao) = aerosol(i,nao) + ZA(ii+(jj-1)*nbin_aer)
            foa(nesp_pom + j,i) = ZA(ii+(jj-1)*nbin_aer)
            ftot = ftot + ZA(ii+(jj-1)*nbin_aer)
         ENDDO
         DO j = 1,nesp_pankow
            jj = pankow_species(j)
            aerosol(i,nao) = aerosol(i,nao) + ZA(ii+(jj-1)*nbin_aer)
            foa(nesp_pom + nesp_aec + j,i) = ZA(ii+(jj-1)*nbin_aer)
            ftot = ftot + ZA(ii+(jj-1)*nbin_aer)
         ENDDO
...
C     ------------------------------------------------------------------
     ...
     &        aerosol(i,nao)+aerosol(i,nae)+aerosol(i,nar)
         if(INUM.EQ.1) then     ! Case when the number concentration is followed
            ...
            dold(i) = (totmass(i)/(cst_pi6*density_aer(i)
     &           *numberconc(i)))**cst_FRAC3
             else
            ...
            if (idens .eq. 1) then !! YK (when density is not fixed)
...
C     -----------------------------------------
     ...
     $        / cst_pi6)**cst_FRAC3
      ...
      IF (IREDIST .EQ. 1 .OR. IREDIST .EQ. 2) then ! number-conserving or interpolation
     ...
     &        fixed_diameter,fixed_density_aer,IDENS,IREDIST,
     &        section_pass,liquid_density,dqlimit,aerosol,
     &        numberconc,totmass)
      else
...
C     -------------------------------------------------
      ...
      do i = 1, nbin_aer
         ...
         do j = 1, Nesp_pom
            jj = poa_species(j)
            ZA( ii + (jj-1) * nbin_aer) = aerosol(i, nao) * foa(j, i)
         enddo
         DO j=1,nesp_aec
            jj=aec_species(j)
            ZA(ii+(jj-1)*nbin_aer)=aerosol(i,nao)*foa(nesp_pom + j,i)
         ENDDO
         DO j=1,nesp_pankow
            jj=pankow_species(j)
            ZA(ii+(jj-1)*nbin_aer)=aerosol(i,nao)
...
C     Conversion from mol.L-1 to \mu g.m-3.
     ...
     $        * scav_frac * 0.7901d0 ! Remarks: what is the meaning of the coefficient 0.7901d0?
         ...
         DO i=ifirstact,nbin_aer ! number concentration.
