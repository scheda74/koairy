...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE STEP(nesp_aer,qn,qext,qti,qinti,
...
C------------------------------------------------------------------------
      ...
      INTEGER nesp_aer
      DOUBLE PRECISION qn,qext(nesp_aer),qti
      DOUBLE PRECISION qinti(NINTIS),qgeqi(nesp_aer)
...
!     if no inorg mass avoid calling thermo
      IF (qtinorg.GT.TINYM) THEN
         CALL EQINORG(nesp_aer,
     &        qext,             ! ext inorg aero conc (µg.m-3)
