...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE KERCD2(nesp_aer,nbin_aer,is1,is2,neq,q,iq,QT,
...
C------------------------------------------------------------------------
      ...
      INTEGER nbin_aer,nesp_aer
      INTEGER is1,is2,neq,iq(nesp_aer,nbin_aer)
      DOUBLE PRECISION q(neq)

      INTEGER js,jesp,jj
      DOUBLE PRECISION qext(nesp_aer)
      DOUBLE PRECISION qTMPA(nesp_aer),QINTMPA(NINTIS)
      DOUBLE PRECISION QGEQTMPA(nesp_aer),KERCDTMPA(nesp_aer)
      DOUBLE PRECISION AATMPA(nesp_aer),CKVTMPA(nesp_aer)

      DOUBLE PRECISION QT(nbin_aer)
      DOUBLE PRECISION VSW(nbin_aer),DSW(nbin_aer)
      DOUBLE PRECISION QINT(NINTIS,nbin_aer),QGEQ(nesp_aer,nbin_aer)
...
C     Compute c/e kernels in each section.
     ...
     &        QT(js).GT.TINYM ) THEN
            ...
            CALL KERCOND( nesp_aer,
     &           q(js), ! num aero conc (#aero.m-3)
