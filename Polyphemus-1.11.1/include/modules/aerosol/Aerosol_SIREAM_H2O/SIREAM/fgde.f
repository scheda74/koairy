...
C     Compute the physical processes
      ...
      IF (ICG2.EQ.1) CALL FCOAG(neq,nesp_aer,nbin_aer,q,iq,dqdt,
     s     couples_coag,first_index_coag,second_index_coag,
     s     coefficient_coag,QT,XSF,MSF,DSF,XSD,MSD,DSD, bin_density)

      IF (ICE2.EQ.1) CALL FCOND(neq,nesp_aer,nbin_aer,q,iq,dqdt,QT,
     s     XSF,MSF,DSF,XSD,MSD,DSD,AA)

      IF (INU2.EQ.1) CALL FNUCL(neq,nesp_aer,nbin_aer,q,iq,dqdt)
