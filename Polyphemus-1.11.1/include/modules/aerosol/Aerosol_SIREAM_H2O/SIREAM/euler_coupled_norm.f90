SUBROUTINE EULER_COUPLED_NORM(ns, nesp,eh2o, dbound, grand, d, diam, &
     fixed_density_aer, idens,kloc, alpha, LMD,DQLIMIT, Q_esp, N)
...
!!$------------------------------------------------------------------------
  ...
  DO jesp = 1, nesp
     DO k = 1, ns
        QTOTin(jesp) = QTOTin(jesp) + Q_esp(k, jesp)
     enddo
  enddo
...
!!!!!!!!! ******** Normalisation
  ...
  DO jesp = 1, nesp
     do k = 1, ns
        QTOTout(jesp) = QTOTout(jesp) + Q_esp(k, jesp)
     enddo
  enddo
     
  do k = 1, ns 
     DO jesp=1, nesp
        if (abs(QTOTin(jesp)-QTOTout(jesp)) .GT. DQLIMIT .and. QTOTout(jesp) .GT. 0d0 ) then
           !print*, "normalisation : ", k
           Q_esp(k, jesp) = Q_esp(k, jesp)*QTOTin(jesp)/QTOTout(jesp)
           N_esp(k, jesp) = N_esp(k, jesp)*QTOTin(jesp)/QTOTout(jesp)
        endif
     enddo
  ENDDO
  N = 0.D0
...
!!$  ENDIF
 ...
 !tests de limite minimum pour le nombre et le volume
  ...
  DO k=1, ns
     ...
     if (IDENS .EQ. 1) CALL COMPUTE_DENSITY(ns,nesp,eh2o, TINYM, Q_esp,LMD,k,rho(k))
  ENDDO
  CALL TEST_MASS_NB(ns,nesp,Q,N,Q_esp)
