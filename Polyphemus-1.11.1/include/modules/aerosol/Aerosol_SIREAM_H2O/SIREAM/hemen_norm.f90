SUBROUTINE HEMEN_NORM(ns, nesp, EH2O, grand, section_pass, dbound, &
     X,diameter_before_redist, fixed_diameter, fixed_density_aer, idens, &
     kloc, alpha, LMD, DQLIMIT,rho, Qesp, N)
...
!!! ~~~~~ alpha is a fraction of each species except water
  ...
  DO k = 1,ns
     DO jesp = 1, nesp
      ...
      if (idens .eq. 1) CALL COMPUTE_DENSITY(ns, nesp,eh2o, TINYN, N_esp, LMD, k, rho(k))
  ENDDO
...
!!! ~~~~~~~~~~~~~~~~ redistribute according to Euler number algorithm ~~~~~~~~~~~~~~~~~~~
     IF(kloc(k) .LE. section_pass-1) THEN
        ...
        IF (grand(k) == 0) THEN 
           ...
           ENDIF
           IF (abs(frac) .GT. 1 .or. isNaN(frac)) THEN
...
!!! rho(k) is the density after condensation/evaporation and before redistribution
  ...
  DO k = section_pass+1 , ns
     if (idens .eq. 1) CALL COMPUTE_DENSITY(ns, nesp, eh2o, TINYM, Qesp, LMD, k, rho(k))
  ...
  do k =1, ns
     if (Q(k) .GT. TINYM .and. N(k) .GT. TINYN) then
        ...
        if (dnew(k) < dbound(k) .or. dnew(k) > dbound(k+1)) then
           ...
           print *, "dnew", dnew(k), "dbound-", dbound(k), "dbound+", dbound(k+1), &
                "dold", fixed_diameter(k)
...
!******** Normalization
  ...
  ! test
  do k =1, ns
     if (abs(N(k)) .GT. TINYN) then
        if (idens .eq. 1) CALL COMPUTE_DENSITY(ns, nesp, eh2o, TINYM, Qesp, LMD, k, rho(k))
        dnew(k) = (Q(k) * 6.D0 /(PI * rho(k) * N(k)))**(1./3.)
     else
        dnew(k) = fixed_diameter(k)
     endif
     if ((dnew(k) .LT. dbound(k)) .or. (dnew(k) .GT. dbound(k+1))) then
        print *,k, "hemen dnew hors de sa boite ", dnew(k), &
             "dnew before normalisation", dnew0(k), "dold", fixed_diameter(k), &
             "b-", dbound(k) , "b+",dbound(k+1), "rho", rho(k), &
             "Q", Q(k), "N",N(k)
     endif
