...
for (i = i1 - 1; i < i2; i++)
  for (j = j1 - 1; j < j2; j++)
    for (k = 0; k < Nz; k++)
      pH(k, j, i) = *(shared_memory.GetData(c) + l++);

for (i = i1 - 1; i < i2; i++)
  for (j = j1 - 1; j < j2; j++)
    for (s = 0; s < Ns_aer; s++)
      for (b = 0; b < Nbin_aer; b++)
        InCloudWetDepositionFlux_aer(s, b, j, i) = *(shared_memory.GetData(c)
                                                     + l++);

for (i = i1 - 1; i < i2; i++)
  for (j = j1 - 1; j < j2; j++)
    for (s = 0; s < Ns; s++)
      InCloudWetDepositionFlux(s, j, i) = *(shared_memory.GetData(c) + l++);
