...
c           Also, the access to the cmn_JVdat.INC is 'read-only' after init.
c           This should enable safe openMP/MPI coding.
c
c common files and what they mean:
c   parm_CTM.INC  dimensions & params for code (CTM and fast-JX)
c   parm_MIE.INC  dimensions for mie code variables.
c   cmn_metdat.INC  CTM 3-D arrays, time of day, grid,  etc.
c   cmn_JVdat.INC   Xsects, Mie, etc., (initialized and then read-only)
c
...
c              COMMON BLOCKS: cmn_metdat.INC
c
c     SET_AER(GMTAU)
...
c     BLOCKS: cmn_metdat.INC, cmn_JVdat.INC Input files: ECT42_grid.dat
c
c     RD_JS(NJ1,NAMFIL):  Read labels of photo. rates, called once by INPHOT.
c     COMMON BLOCKS: cmn_metdat.INC, cmn_JVdat.INC
...
c     N.B. all these need access to cmn_JVdat.INC, but do NOT write into it.
c     also have no need to access cmn_metdat.INC
c
c     JRATET(PPJ,TTJ,FFF, VALJL):  Calculate J-value, called by PTOTOJ.
c     COMMON BLOCKS: cmn_JVdat.INC
c
...
c
c
c     RD_XXX(NJ1,NAMFIL,JFACTA):  Read wavelength bins, solar fluxes, Rayleigh
...
c     include 'parm_MIE.INC' = dimension parameters
c
c     BLKSLV (FJ,POMEGA,FZ,ZTAU,ZFLUX,ZREFL,WT,EMU,PM,PM0,M,N,MFIT,ND)
c     PARAMETER FILE: parm_MIE.INC
c
...
c
c-----------------------------------------------------------------------
      subroutine fastjx (NxStep, NyStep, NzStep, X_MIN, DELTA_X, Y_MIN,
     $     DELTA_Y,  GridZInterf,CloudOpticalDepth, IceOpticalDepth,
     $     SurfacePressure, AerosolOpticalDepth, SingleScatteringAlbedo,
     $     ExtinctionEfficiencyFacor, PhaseFunction,
     $     PhotolysisRate, PhotolysisNameList,  NPHOT, YEAR,
     $     IDAY, HOUR, DirectoryParameter, DirectoryParameter_len)
c-----------------------------------------------------------------------
c     Subroutine linking FAST-JX with POLYPHEMUS Cloud diagnosis and Aerosol
c     simulated concentrations (Real 10/08).
c     CloudOpticalDepth and IceOpticalDepth must be pre-calculated in the
c     preprocessing stage.  AerosolOpticalDepth must be given at 600 nm.
c
...
c
      ...
      include 'parm_CTM.INC'
      include 'cmn_metdat.INC'
      include 'cmn_JVdat.INC'

      real X_MIN, DELTA_X, Y_MIN, DELTA_Y
      integer  NPHOT
      real HOUR

      real  ZPJ(NzStep,NPHOT)   !2-D array of J's indexed to CTM chemistry!
      real  FREFL,U0            ! fraction of flux(energy-wtd) reflected
      real  GMTAU,TINIT(NzStep),ODINIT(NzStep),ICEINIT(NzStep),ALBEDO
      real SZA
      integer I,J,K,L,Q,ILNG,JLAT,ODINDX,ICEINDX
      integer YEAR, DAY, LEAP, IDAY
c     ------------------------------------------------
c
      integer NxStep, NyStep, NzStep
      integer, parameter :: NSTEP=1
      real PhotolysisRate(NxStep,NyStep,NzStep,NPHOT)
      real CloudOpticalDepth(NxStep,NyStep,NzStep)
      real IceOpticalDepth(NxStep,NyStep,NzStep)
      real AerosolOpticalDepth(NxStep,NyStep,NzStep)
      real SingleScatteringAlbedo(NxStep,NyStep,NzStep,4)
      real ExtinctionEfficiencyFacor(NxStep,NyStep,NzStep,4)
      real PhaseFunction(8,NxStep,NyStep,NzStep,4)
      real SurfacePressure(NxStep,NyStep)
      real GridZInterf(NzStep+1)

      real SingleScatteringAlbedo_TMP(NzStep+1,4)
      real ExtinctionEfficiencyFacor_TMP(NzStep+1,4)
      real PhaseFunction_TMP(8,NzStep+1,4)

      character(LEN=DirectoryParameter_len) DirectoryParameter
      integer DirectoryParameter_len

      character (LEN=10) PhotolysisNameList(NPHOT)
      integer JIND(NPHOT)
      real JFACTA(X_)
...
c     Check that Dimensions NxStep, NyStep, NzStep < IMAX, JMAX, LMAX

      if (NxStep.gt.IMAX) then
         write(6,101), NxStep, IMAX
         stop
      endif

      if (NyStep.gt.JMAX) then
         write(6,102), NyStep, JMAX
         stop
      endif

      if (NzStep.gt.LMAX) then
         write(6,103), NzStep, LMAX
         stop
      endif

c     Check that the number of input photolysis rates NPHOT = number of photolyis
c     rates in Fast-J

c$$$  if (NPHOT.ne.JVN_) then
c$$$  write(6,*) 'FAST-J ERROR: Input number of photolysis rates
c$$$  & NPHOT is equal to', NPHOT, ', it must be equal to the parameter
c$$$  & JVN_ (see file parm_CTM.INC). JVN_ = ', JVN_
c$$$  stop
c$$$  endif
...
c     Set constant values
      ...
      do L=1,L1_
         ZCTM(L)=GridZInterf(L)
      enddo

      do J=1,J_
         do I=1,I_
            PMEAN(I,J)=SurfacePressure(I,J)/1d2
         enddo
      enddo
c
      do K=1,L_
         ODINIT(K)=CloudOpticalDepth(1,1,K)
         ICEINIT(K)=IceOpticalDepth(1,1,K)
         TINIT(K)=230           ! Never used in FastJ calculation -
...
C-----------------------------------------------------------------------
      call INPHOT(X_MIN, DELTA_X, Y_MIN, DELTA_Y,
     $     DirectoryParameter, DirectoryParameter_len,
     $     NPHOT,PhotolysisNameList,JIND,JFACTA)
C-----------------------------------------------------------------------
      call SET_CLD0(TINIT,ODINIT,ICEINIT,ODINDX,ICEINDX,ALBEDO)
C-----------------------------------------------------------------------
      call SET_ATM0
C-----------------------------------------------------------------------
      call SET_AER0(AerosolOpticalDepth)
C-----------------------------------------------------------------------

c     Loop over time + grid boxes
c     ---------------------------
...
c     IDAY=(DAY+91)-(MONTH*3055)/100
         MONTH=MONTH-2
         write(*,*) 'MONTH or DAY updated=', MONTH, IDAY
      endif
...
c     In case there is only one time_step, no need to re-read OD
      call SET_AER(SurfacePressure)

      call SET_CLD(CloudOpticalDepth,IceOpticalDepth)
C-----------------------------------------------------------------------
...
c     Loop over grid boxes
      do J=1,J_
         do I=1,I_
            ...
            do K=1,4
               do L=1,L_
                  ExtinctionEfficiencyFacor_TMP(L,K)=
     $                 ExtinctionEfficiencyFacor(I,J,L,K)
                  SingleScatteringAlbedo_TMP(L,K)=
     $                 SingleScatteringAlbedo(I,J,L,K)
                  do Q=1,8
                     PhaseFunction_TMP(Q,L,K)=
     $                    PhaseFunction(Q,I,J,L,K)
                  enddo
               enddo
               SingleScatteringAlbedo_TMP(L1_,K)=1.d0
               ExtinctionEfficiencyFacor_TMP(L1_,K)=1.d0
               do Q=1,8
                  PhaseFunction_TMP(Q,L1_,K)=0.d0
               enddo
            enddo
C-----------------------------------------------------------------------
            call PHOTOJ(GMTAU,IDAY,ILNG,JLAT,SZA,U0,FREFL,ZPJ,
     $           ExtinctionEfficiencyFacor_TMP,
     $           SingleScatteringAlbedo_TMP,PhaseFunction_TMP,
     $           NPHOT,JIND,JFACTA)
C-----------------------------------------------------------------------

            do L=1,JVL_
               do K=1,NPHOT
                  PhotolysisRate(I,J,L,K)=ZPJ(L,K)
               enddo
            enddo
 ...
 101  format(' Input dimensions Nx= ',i3,/,
     &     ' Maximum dimension allowed (IMAX)= ',i3,
     &     ' If you want to use Nx > IMAX you must change IMAX'
     &     ' in parm_CTM.INC and recompile')
 102  format(' Input dimensions Ny= ',i3,/,
     &     ' Maximum dimension allowed (JMAX)= ',i3,
     &     ' If you want to use Ny > JMAX you must change JMAX'
     &     ' in parm_CTM.INC and recompile')
 103  format(' Input dimensions Nz= ',i3,/,
     &     ' Maximum dimension allowed (LMAX)= ',i3,
     &     ' If you want to use Nz > LMAX you must change LMAX'
     &     ' in parm_CTM.INC and recompile')
...
c-------calculate effective altitude of each CTM level edge
      ...
      subroutine SET_AER(SurfacePressure)
...
c     Test-
      real, intent(in) :: SurfacePressure(I_,J_)
      ...
      subroutine SET_CLD(CloudOpticalDepth,IceOpticalDepth)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      real, intent(in) :: CloudOpticalDepth(I_,J_,LWE_)
      real, intent(in) :: IceOpticalDepth(I_,J_,LWE_)
      integer  I, J, L
c
      ...
      do J = 1,J_
         do I = 1,I_
            do L = 1,LWE_
               ODCLD(I,J,L) = CloudOpticalDepth(I,J,L)
               ICECLD(I,J,L) = IceOpticalDepth(I,J,L)
            enddo
            ODCLD(I,J,L1_)  = 0.d0
            ICECLD(I,J,L1_)  = 0.d0
         enddo
...
c-----------------------------------------------------------------------
      subroutine INPHOT(X_MIN, DELTA_X, Y_MIN, DELTA_Y,
     $     DirectoryParameter, DirectoryParameter_len,
     $     NPHOT,PhotolysisNameList,JIND,JFACTA)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
c
c     Elsa 08 : For link with the c++ routines, 4 arguments have been added:
c     X_MIN, DELTA_X, Y_MIN and DELTA_Y that described the CTM horizontal grids
c     X_MIN, DELTA_X: minimum longitude and longitude gridsize of the CTM grid.
c     Y_MIN, DELTA_Y: minimum latitude and latitude gridsize of the CTM grid.
c
...
C-----------------------------------------------------------------------
c
c  ELSA - Modfified to read Polyphemus CTM Grid instead of T42 Grid
...
c Read in annual mean pressure field
      ...
      do I = 1,I_
        XDGRD(I)=X_MIN+(I-1)*DELTA_X
        XGRD(I) = XDGRD(I) *PI/180.d0
      enddo
      do J = 1,J_
         YDGRD(J)=Y_MIN+(J-1)*DELTA_Y
         YGRD(J) = YDGRD(J) *PI/180.d0
      enddo

c
c      BaseDir='../Polyphemus-100608-aerosolOD/include/fastJX/'
      DirectoryParameter=trim(DirectoryParameter)
c Read in fast-J X-sections (spectral data) <<<<<<<<<<<<<< new fast-JX

      call RD_XXX(IPH,DirectoryParameter//'FJX_spec.dat',JFACTA)

c Read in aerosol/cloud scattering data <<<<<<<<<<<<<<<<<< new fast-JX

      call RD_MIE(IPH,DirectoryParameter//'FJX_scat.dat')

c Read in labels of photolysis rates required   >>>>> keyed to chem code
      call RD_JS(IPH,PhotolysisNameList,NPHOT,JIND)
c
c Read in T & O3 climatology                    >>>> general backup clim.

      call RD_PROF(IPH,DirectoryParameter//'atmos_std.dat')
...
c$$$      subroutine RD_JS(NJ1,NAMFIL)
c$$$C-----------------------------------------------------------------------
c$$$c  Reread the ratj.dat file to map photolysis rate to reaction
c$$$c  Read in quantum yield 'jfacta' and fastj2 label 'jlabel'
c$$$C-----------------------------------------------------------------------
c$$$c
c$$$c     jfacta    Quantum yield (or multiplication factor) for photolysis
c$$$c     jlabel    Reference label identifying appropriate J-value to use
c$$$c     ipr       Photolysis reaction counter - should total 'JVN_'
c$$$c
c$$$C-----------------------------------------------------------------------
c$$$      implicit none
c$$$
c$$$      include 'parm_CTM.INC'
c$$$      include 'cmn_metdat.INC'
c$$$      include 'cmn_JVdat.INC'
c$$$
c$$$      integer, intent(in) ::  NJ1
c$$$      character(*), intent(in) ::  NAMFIL
c$$$
c$$$      integer  IPR, J, K
c$$$      character*120 CLINE
c$$$c
c$$$c Reread the ratj.dat file to map photolysis rate to reaction
c$$$c                     Read in quantum yield jfacta and fastj2 label jlabel
c$$$      IPR = 0
c$$$      open (NJ1,file=NAMFIL,status='old',form='formatted')
c$$$ 10   read (NJ1,'(A)',err=20)  CLINE
c$$$      if (IPR .eq. JVN_) goto 20
c$$$
c$$$      if (CLINE(2:5).eq.'9999') then
c$$$        go to 20
c$$$      elseif (CLINE(1:1).eq.'#') then
c$$$        go to 10
c$$$      elseif (CLINE(5:5).eq.'$') then
c$$$        go to 10
c$$$      else
c$$$        IPR = IPR+1
c$$$        read (CLINE(79:83),'(F5.1)') JFACTA(IPR)
c$$$        read (CLINE(86:92),'(A7)')   JLABEL(IPR)
c$$$        JFACTA(IPR) = JFACTA(IPR)/100.d0
c$$$        go to 10
c$$$      endif
c$$$ 20   close(NJ1)
c$$$
...
c$$$C-----------------------------------------------------------------------
c$$$c  compare Xsections titles with J-values listed in chem code (jratd.dat)
c$$$c  map the J-values needed for chemistry (ratj.dat) onto the fast-JX rates
c$$$c  >>>>>>>>>>>>>>>>current code revised to JPL-02 ver 8.5 (5/05)<<<<<<<<<
c$$$c          >>>this must now follow the read in of Xsects, etc<<<
c$$$C-----------------------------------------------------------------------
c$$$
c$$$c---Zero / Set index arrays that map Jvalue(j) onto rates
c$$$      do J = 1,JVN_
c$$$        JIND(J) = 0
c$$$      enddo
c$$$      do J = 1,NJVAL
...
c$$$      end
      ...
      subroutine RD_JS(NJ1,PhotolysisNameList,NPHOT,JIND)
C-----------------------------------------------------------------------
c  Map photolysis rate to reactions read in FJX_spec.dat
C-----------------------------------------------------------------------
c
c
C-----------------------------------------------------------------------
...
c---Zero / Set index arrays that map Jvalue(j) onto rates
      ...
      do J = 1,NJVAL
         do K = 1,NPHOT
            if (PhotolysisNameList(K) .eq. TITLEJ(J)) JIND(K)=J
         enddo
      enddo
      ...
      do K=1,NPHOT
        J = JIND(K)
        if (J.eq.0) then
         write(6,'(i5,a10,a28)') K,PhotolysisNameList(K),
...
c-----------------------------------------------------------------------
      subroutine SET_CLD0(TINIT,ODINIT,ICEINIT,ODINDX,ICEINDX,ALBEDO)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      integer, intent(in) ::  ODINDX,ICEINDX
      real, intent(in) :: TINIT(L_),ODINIT(L_),ICEINIT(L_),ALBEDO
      integer  I, J, L
c
      do J = 1,J_
      do I = 1,I_
...
C-----------------------------------------------------------------------
      subroutine SET_AER0(AerosolOpticalDepth)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      integer I, J, L
      real AerosolOpticalDepth(I_,J_,L_)
...
c    DAERn(L1_=L_+1)= aerosol/cloud OD in layer above CTM (L1_)
      ...
      do J = 1,J_
         do I = 1,I_
            do L = 1,L_
               DAER1(I,J,L) = AerosolOpticalDepth(I,J,L)
               DAER2(I,J,L) = 0.d0
            enddo
            DAER1(I,J,L1_) = 0.d0
            DAER2(I,J,L1_) = 0.d0
...
C-----------------------------------------------------------------------
      ...
      integer  I, J, K, L, M, N
      real   PSTD(52),OREF2(51),TREF2(51),PJ(L1_+1)
      real   DLOGP,F0,T0,PB,PC,XC,MASFAC,SCALEH
c
...
C-----------------------------------------------------------------------
      ...
      include 'parm_CTM.INC'
      include 'parm_MIE.INC'
      include 'cmn_metdat.INC'
      include 'cmn_JVdat.INC'

      real, intent(in)  ::  UTIME,SZA
      integer,intent(in)  ::  IDAY,ILNG,JLAT
      real, intent(in)  ::  SingleScatteringAlbedo_TMP(L1_,4)
      real, intent(in)  ::  ExtinctionEfficiencyFacor_TMP(L1_,4)
      real, intent(in)  ::  PhaseFunction_TMP(8,L1_,4)
      real, intent(out) ::  ZPJ(JVL_,NPHOT)   !2-D array of J's indexed to CTM chemistry!
      real, intent(out) ::  FREFL,U0         !fraction of energy reflected

      integer NPHOT
      integer, intent(in) :: JIND(NPHOT)
      real, intent(in) :: JFACTA(X_)

c-----------------------------------------------------------------------

c--------key amtospheric data needed to solve plane-parallel J---------
      real, dimension(5,L1_) :: AER
      integer,dimension(5,L1_) :: ADX
      real, dimension(L1_+1) :: ABX, TTJ,DDJ,ZZJ,ZHL
      real, dimension(L1_+1) :: PPJ
      integer,dimension(L2_+1) :: JXTRA

      real             :: RFLECT,SOLF,FREFS,FREFI,FJTOP,FJBOT,FSBOT
      real             :: FJFLX(L_),FLXD(L1_),FLXD0
      real             :: AMF(L1_+1,L1_+1)
c------------key arrays AFTER solving for J's---------------------------
      real  FFF(W_,JVL_)
      real  FLXUP(W_),FLXDN(W_),DIRUP(W_),DIRDN(W_)
      real  VALJL(JVL_,NJVAL) !2-D array of J_s returned by JRATET

      integer  J,K,L,KM, RATIO(W_)
      real   AVGF(L_),XQO3,XQO2    ,WAVE, FLINT, TTT
      real   MASFAC,SCALEH

c---flux/heating arrays (along with FJFLX,FLXD,FLXD0)
      real  FLXJ(L1_),FFX(W_,L1_),FFXNET(W_,8),FFX0,FXBOT,FABOT
C-----------------------------------------------------------------------
...
c---load the amtospheric column data
      PPJ(1) = PMEAN(ILNG,JLAT) ! only usefull if Z-coord
      MASFAC = 100.d0*6.022d+23/(28.97d0*9.8d0*10.d0)
      do L = 1,L1_
c -- Sigma-coord
c        PPJ(L) = ETAA(L) + ETAB(L)*P(ILNG,JLAT)
c -- Z-coord
        SCALEH      = 1.3806d-19*MASFAC*TJ(ILNG,JLAT,L)
...
c     AER(3,L)  = DAER3(ILNG,JLAT,L) ! Opt. Depth aerosol 3 in layer L
         AER(3,L)  = ICECLD(ILNG,JLAT,L) ! Ice Cloud Opt. Depth in L
...
c     ADX(3,L)  = NAER3(ILNG,JLAT,L)  ! index for aerosol 3 at layer L
         ADX(3,L)  = NICEX(ILNG,JLAT,L) ! index for ice cloud at layer L
...
c      do L = 1,L1_
c       do I=1,4
c        ADX(I,L) = min(NAA, max(0, ADX(I,L)))
c       enddo
c      enddo
...
C-----------------------------------------------------------------------
      call OPMIE (K,KM,ABX,AER,ADX,U0,RFLECT,AMF,JXTRA,AVGF,
     &       FJTOP,FJBOT,FSBOT,FJFLX,FLXD,FLXD0,
     &       ExtinctionEfficiencyFacor_TMP,SingleScatteringAlbedo_TMP,
     &       PhaseFunction_TMP)
...
c---map the J-values from fast-JX onto ASAD ones (use JIND & JFACTA)
      do L = 1,JVL_
        do J = 1,NPHOT
           if (JIND(J).gt.0) then
              ZPJ(L,J) = VALJL(L,JIND(J))*JFACTA(JIND(J))
           else
              ZPJ(L,J) = 0.d0
           endif
        enddo
...
c      call JP_ATM(PPJ,TTJ,DDJ,ZZJ,ZHL,ZZHT,AER,ADX,JXTRA)
...
C-----------------------------------------------------------------------
      ...
      real, intent(in)  ::  PPJ(L1_+1),TTJ(L1_)
      real, intent(in)  ::  FFF(W_,JVL_)
      real, intent(out) ::  VALJL(JVL_,NJVAL)

      real  FLINT             ! external function for X-sections
      real  VALJ(X_)          ! temp for call J's at one L
      real  QO2TOT, QO3TOT, QO31DY, QO31D, QQQT, TFACT
      real  TT,PP,DD,TT200,TFACA,TFAC0,TFAC1,TFAC2,QQQA,QQ2,QQ1A,QQ1B
      integer J,K,L, IV
c
...
c            QO31D = QO31DY
        ...
        do J = 4,NJVAL
          ...
          if (TQQ(2,J) .gt. TQQ(1,J)) then
           TFACT =
     &      max(0.d0,min(1.d0,(TT-TQQ(1,J))/(TQQ(2,J)-TQQ(1,J))))
          else
           TFACT = 0.d0
          endif
...
c-----------------------------------------------------------------------
      subroutine JP_ATM(PPJ,TTJ,DDJ,ZZJ,ZHL,ZZHT,AER,ADX,JXTRA)
c-----------------------------------------------------------------------
      implicit none
...
c--------key amtospheric data needed to solve plane-parallel J---------
      real, dimension(5,L1_) :: AER
      integer,dimension(5,L1_) :: ADX
      real, dimension(L1_+1) :: TTJ,DDJ,ZZJ,ZHL
      real, dimension(L1_+1) :: PPJ
      integer,dimension(L2_+1) :: JXTRA
      real                   :: ZZHT
c-----------------------------------------------------------------------
      integer  I, L
      real   COL(4),COLO2,COLO3,ZKM,DELZ,ZTOP
...
C-----------------------------------------------------------------------
      subroutine RD_XXX(NJ1,NAMFIL,JFACTA)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      implicit none
      real, intent(in) ::   GMTIME,YGRDJ,XGRDI
      integer, intent(in) ::  NDAY
      real, intent(out) ::  SZA,COSSZA,SOLFX

      real  PI, PI180, LOCT
      real  SINDEC, SOLDEK, COSDEC, SINLAT, SOLLAT, COSLAT
c
      PI     = 3.141592653589793d0
      PI180  = PI/180.d0
...
C-----------------------------------------------------------------------
      ...
      real, intent(in)  ::   GMU,RAD,ZHL(L1_+1)
      real, intent(out) ::   AMF(L1_+1,L1_+1)
...
c     XL      Slant path between points
      ...
      integer  I, J, II
      real   XMU1,XMU2,XL,DIFF,TANHT,RZ(L1_+1),RQ(L1_+1)
c
      RZ(1) = RAD + ZHL(1)
...
c
      ...
      real,  intent(in) ::  AER(5,L1X)      !cloud+3aerosol OD in each layer
...
c
      ...
      real  DTAUX(L1X),TTAU(L2X+1),DTAUJ, ATAU1,ATAULN,ATAUM,ATAUN1
c
...
c    do NOT count if ADX = 0
          DTAUX(L)   = 0.d0
        do I = 1,4
          if (ADX(I,L) .ne. 0) DTAUX(L) = DTAUX(L) + AER(I,L)
        enddo
      enddo
c
...
C-----------------------------------------------------------------------
      subroutine OPMIE (KW,KM,ABX,AER,ADX,U0,RFLECT,AMF,
...
C-----------------------------------------------------------------------
      ...
      real, intent(in) ::   AER(5,L1_),ABX(L1_),AMF(L1_+1,L1_+1)
      real, intent(in) ::   U0,RFLECT
      integer, intent(in) ::  KW,KM,ADX(5,L1_)
      integer, intent(in) ::  JXTRA(L2_+1)
      real, intent(in) :: SingleScatteringAlbedo_TMP(L1_,4)
      real, intent(in) :: ExtinctionEfficiencyFacor_TMP(L1_,4)
      real, intent(in) :: PhaseFunction_TMP(8,L1_,4)
      real, intent(out) ::  FJACT(L_),FJTOP,FJBOT,FSBOT
      real, intent(out) ::  FJFLX(L_),FLXD(L1_),FLXD0
c
      ...
      real   SUMT,SUMJ,FJACT2(L_)
c
      real  QXMIE(5),XLAER(5),SSALB(5),DTAUX(L1_+1),PIAER(5,L1_)
     &       ,POMEGAJ(2*M_,L2_+1),TTAU(L2_+1),FTAU(L1_+1)
     &       ,FTAU2(L2_+1)
     &       ,XLTAU,TAUDN,TAUUP
     &       ,DTAUJ,TAUBTM,TAUTOP,FBTM,FTOP,POMEGAB(2*M_)
     &       ,ATAUA,ATAUZ
c--- variables used in mie code-----------------------------------------
      real   FJ(N_),POMEGA(2*M_,N_),FZ(N_),ZTAU(N_),ZREFL,ZU0,ZFLUX
      real   FJT,FJB, FJFLX0
      integer  MFIT, ND
...
C---  for Mie code scale extinction at 600 nm to wavelength WAVEL (QXMIE)
            ...
            if (ADX(I,L) .lt. 0) then
               QXMIE(I)=ExtinctionEfficiencyFacor_TMP(L,I)/
     $              ExtinctionEfficiencyFacor_TMP(L,3)
               SSALB(I)=SingleScatteringAlbedo_TMP(L,I)
            endif
         enddo
         ...
         do I = 1,MFIT
            ...
            do K = 1,5
               if (ADX(K,L) .gt. 0)  then
                  POMEGAJ(I,L)=POMEGAJ(I,L)
     $             + PIAER(K,L)*PAA(I,KM,ADX(K,L))
               endif
               if (ADX(K,L) .eq. -1)   then
                  POMEGAJ(I,L)=POMEGAJ(I,L) + PIAER(K,L)*
     $         PhaseFunction_TMP(I,L,KM)
               endif
            enddo
         enddo
      enddo
...
C-----------------------------------------------------------------------
      implicit none
      include 'parm_MIE.INC'
c--- expect parameters M_, N_ in parm_MIE.INC------------------------------
c
      integer, intent(in) ::  MFIT, ND
      real, intent(in)  ::  POMEGA(2*M_,N_),FZ(N_),ZTAU(N_)
     &                       ,ZREFL,ZU0,ZFLUX
      real, intent(out) ::  FJ(N_),FJT,FJB
c
      real  WT(M_),EMU(M_),PM(M_,2*M_),PM0(2*M_),CMEQ1
      integer I, IM, M, N
C-----------------------------------------------------------------------
...
c
     ...
     &                       ,ZFLUX,ZREFL
      real, intent(out) ::  FJ(N_),FJTOP,FJBOT
c
      real, dimension(M_,N_)    :: HH, RR
      real, dimension(M_,M_,N_) :: AA, BB, CC, DD
      integer I, ID
      real  FIPLUS
C-----------------------------------------------------------------------
...
c
     ...
     &                       ,WT(M_),EMU(M_),PM(M_,2*M_),PM0(2*M_)
     &                       ,ZFLUX,ZREFL
      real, intent(out) ::  BB(M_,M_,N_),AA(M_,M_,N_),CC(M_,M_,N_)
     &                       ,HH(M_,N_)
c LOCAL:
...
c
      real  S(M_,M_), W(M_,M_), W1(M_,M_), U1(M_,M_), U(M_,M_)
      real  B1(M_,M_),C(M_),C1(M_),H1(M_)
C---------------------------------------------
...
c
      ...
      real, intent(in)  ::  AA(M_,M_,N_),BB(M_,M_,N_),CC(M_,M_,N_)
      real, intent(in)  ::  HH(M_,N_)

      real, dimension(M_,N_)    :: RR
      real, dimension(M_,M_,N_) :: DD
      real, dimension(M_,M_)    :: B
      real, dimension(M_)       :: R
      integer I, J, K, ID
...
C-----------------------------------------------------------------------
      ...
      real, intent(out) :: F(101)     !F(N+1)
      integer I
      real A,DX,D,DSQ,DDP1, B(101),R(101)
C
      if (F0 .eq. 0.d0) then
        do I = 1,N
          F(I)=0.d0
