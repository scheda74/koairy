...
#include "Polair3DTransportSplit.hxx"
...
namespace Polyphemus
{
  ...
  */
  ...
  ::Polair3DTransport(string config_file): BaseModel<T>(config_file)
  {
    ...
    this->D2_map["Rain"] = &Rain_i;
    this->D2_map["Rain_i"] = &Rain_i;
    this->D2_map["Rain_f"] = &Rain_f;

    this->D2_map["CloudHeight"] = &CloudHeight_f;
    this->D2_map["CloudHeight_i"] = &CloudHeight_i;
    this->D2_map["CloudHeight_f"] = &CloudHeight_f;
    ...
    this->D3_map["BoundaryCondition_z"] = &BoundaryCondition_z_i;
    this->D3_map["BoundaryCondition_z_i"] = &BoundaryCondition_z_i;

    this->D3_map["DepositionVelocity"] = &DepositionVelocity_f;
    this->D3_map["DepositionVelocity_i"] = &DepositionVelocity_i;
    this->D3_map["DepositionVelocity_f"] = &DepositionVelocity_f;

    this->D3_map["SurfaceEmission"] = &SurfaceEmission_f;
    this->D3_map["SurfaceEmission_i"] = &SurfaceEmission_i;
    this->D3_map["SurfaceEmission_f"] = &SurfaceEmission_f;
    ...
    this->D4_map["BoundaryCondition_y"] = &BoundaryCondition_y_i;
    this->D4_map["BoundaryCondition_y_i"] = &BoundaryCondition_y_i;

    this->D4_map["BoundaryCondition_x"] = &BoundaryCondition_x_i;
    this->D4_map["BoundaryCondition_x_i"] = &BoundaryCondition_x_i;

    this->D4_map["VolumeEmission"] = &VolumeEmission_f;
    this->D4_map["VolumeEmission_i"] = &VolumeEmission_i;
    this->D4_map["VolumeEmission_f"] = &VolumeEmission_f;

    this->D4_map["ScavengingCoefficient"] = &ScavengingCoefficient_f;
    this->D4_map["ScavengingCoefficient_i"] = &ScavengingCoefficient_i;
    this->D4_map["ScavengingCoefficient_f"] = &ScavengingCoefficient_f;
    ...
    this->field_species["ScavengingCoefficient_i"] = &species_list_scav;
    this->field_species["ScavengingCoefficient_f"] = &species_list_scav;
  ...
  */
  ...
  ::ReadConfiguration()
  {
    ...
    else if (this->scavenging_model == "belot")
      {
	// Initializes a and b coefficients.
	species_data_stream.SetSection("[belot]");
	...
	if (species == "all")
	  {
	    ...
	    for (int i = 0; i < Ns_scav; i++)
	      {
		belot_constant_a[ScavengingName(i)] = a;
		belot_constant_b[ScavengingName(i)] = b;
	      }
	  }
	else
	  while (!species_data_stream.IsEmpty())
	    {
	      species = species_data_stream.GetElement();
	      species_data_stream.GetNumber(belot_constant_a[species]);
	      species_data_stream.GetNumber(belot_constant_b[species]);
	    }
      }
    else if (this->scavenging_model == "constant")
      {
  ...
  ::CheckConfiguration()
  {
	...
	&& this->input_files["meteo"]("CloudHeight").empty())
      throw "Cloud height is needed but no input data file was provided.";
	...
	// scavenged species.
	for (int i = 0; i < Ns_scav; i++)
	  if (belot_constant_a.find(ScavengingName(i))
	      == belot_constant_a.end()
	      || belot_constant_b.find(ScavengingName(i))
	      == belot_constant_b.end())
	    throw string("ERROR! Coefficient a or b for species \"")
	      + ScavengingName(i)
	      + string("\" not found in section [belot].");
      }
    else if (this->scavenging_model == "constant")
      {
    ...
    else if (this->scavenging_model == "none"
	     && this->option_process["collect_wet_flux"])
  ...
  */
  ...
  void Polair3DTransport<T, ClassAdvection, ClassDiffusion>::Allocate()
  {
    ...
    if (this->option_manage["cloud_height"])
      {
	CloudHeight_i.Resize(this->GridY2D, this->GridX2D);
	CloudHeight_f.Resize(this->GridY2D, this->GridX2D);
	FileCloudHeight_i.Resize(this->GridY2D, this->GridX2D);
	FileCloudHeight_f.Resize(this->GridY2D, this->GridX2D);
      }
    ...
    ScavengingCoefficient_i.Resize(GridS_scav, this->GridZ4D,
				   this->GridY4D, this->GridX4D);
    ScavengingCoefficient_f.Resize(GridS_scav, this->GridZ4D,
				   this->GridY4D, this->GridX4D);
  ...
  */
  ...
  void Polair3DTransport<T, ClassAdvection, ClassDiffusion>::Init()
  {
	...
	for (int s = 0; s < Ns_scav; s++)
	  for (int j = 0; j < this->Ny; j++)
		...
		for (int k = 0; k < this->Nz; k++)
		  if (this->CloudHeight_f(j, i) < this->GridZ4D(k))
		    WetDepositionFlux(s, j, i) +=
		      this->Concentration(ScavengingGlobalIndex(s), k, j, i)
		      / this->Delta_t
		      * (1.- exp(- this->Delta_t *
				 ScavengingCoefficient_f(s, k, j, i)))
  ...
  void Polair3DTransport<T, ClassAdvection, ClassDiffusion>::InitStep()
  {
	...
	&& this->scavenging_model != "none")
      {
	ScavengingCoefficient_i.GetArray()
	  = ScavengingCoefficient_f.GetArray();
	if (this->scavenging_model == "microphysical")
	  InitScavengingCoefficient(Temperature_f, Pressure_f,
				    CloudHeight_f, Rain_f,
				    ScavengingCoefficient_f);
	else if (this->scavenging_model == "belot")
	  InitScavengingCoefficient(Rain_f, ScavengingCoefficient_f);
	else if (this->scavenging_model == "pudykiewicz")
	  InitScavengingCoefficient(Temperature_f, Pressure_f,
				    SpecificHumidity_f,
				    ScavengingCoefficient_f);
	else if (this->scavenging_model == "constant")
	  InitScavengingCoefficient(ScavengingCoefficient_f);
      }
  ...
  */
			      ...
			      Data<T, 4>& ScavengingCoefficient_)
  {
    ...
    for (int s = 0; s < Ns_scav; s++)
      {
	T cste_a = this->belot_constant_a[ScavengingName(s)];
	T cste_b = this->belot_constant_b[ScavengingName(s)];
	for (int j = 0; j < Ny; j++)
	  for (int i = 0; i < Nx; i++)
	    if (Rain_(j, i) > 0.)
	      {
  ...
  */
  ...
  //! Performs one diffusion step.
  template<class T, class ClassAdvection, class ClassDiffusion>
  void Polair3DTransport<T, ClassAdvection, ClassDiffusion>::DiffusionXY()
  {
    Diffusion_.ForwardXY(*this);
  }

  //! Performs one diffusion step.
  template<class T, class ClassAdvection, class ClassDiffusion>
  void Polair3DTransport<T, ClassAdvection, ClassDiffusion>::DiffusionZ()
  {
    Diffusion_.ForwardZ(*this);
  }
		  ...
		  for (int k = 0; k < this->Nz; k++)
			...
			scavenging_ratio =
			  exp(- 0.5 * this->Delta_t
			      * (ScavengingCoefficient_i(s, k, j, i)
				 + ScavengingCoefficient_f(s, k, j, i)));
			if(this->option_process["collect_wet_flux"])
			  WetDepositionFlux(s, j, i) +=
  ...
  */
  ...
  void Polair3DTransport<T, ClassAdvection, ClassDiffusion>::Backward()
  {
    ...
    for (int s = 0; s < this->Ns; s++)
      if (HasScavenging(s) && this->scavenging_model != "none")
	{
