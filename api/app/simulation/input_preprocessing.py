...
# path_to_weights,
FILEPATHS = {
    netfile: os.path.dirname(__file__) + "/data/traffic-input/emission_output.xml"
}
...
def write_sumocfg_files(begin=0, end=10800):
    for scenario_id in VALID_SCENARIO_IDS:
        ...
        route_files_tag = ET.SubElement(
            input_tag,
            'route-files',
            {'value': 'custom{0}.rou.xml'.format(scenario_id)}
        )
        ...
        begin_tag = ET.SubElement(time_tag, 'begin', {'value': begin})
        end_tag = ET.SubElement(time_tag, 'end', {'value': end})
        ...
        configuration_file.write('../data/input-simulation/custom{0}.sumocfg'.format(scenario_id))

        # pretty formatting
        parser = etree.XMLParser(remove_blank_text=True)
        tree = etree.parse('../data/input-simulation/custom{0}.sumocfg'.format(scenario_id), parser)
        tree.write('../data/input-simulation/custom{0}.sumocfg'.format(scenario_id), pretty_print=True)
...
def write_random_trips_and_routes(path_to_net,
    ...
    cmd = "python %s -n %s -e %s -o %s --route-file %s --validate --fringe-factor %s -p %s"\
            % ( path_to_script,
                ...
                path_to_route_output, 
                fringe_factor, 
                (simulation_timesteps / (number_of_vehicles * 1.0))
            )
    if (load_weights):
        cmd += " --weights-prefix %s" % path_to_weights
    subprocess.call(cmd.split())
